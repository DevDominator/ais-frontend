...
export const addBuilder = async (data: any) => {
    try {
        const resp = await fetch(`${import.meta.env.VITE_BACKEND_BASE_URL}/api/v1/Builders/`, {
            method: "POST",
            headers: {
...
export const deleteBuilder = async (id: any) => {
    try {
        const resp = await fetch(`${import.meta.env.VITE_BACKEND_BASE_URL}/api/v1/builders/${id}`, {
            method: "DELETE"
        });
